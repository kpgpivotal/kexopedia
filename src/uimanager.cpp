...
UIManager::~UIManager() {
    try {
        if (std::ofstream output{CONFIG_FILE_NAME}) {
            ...
            archive( /*cereal::make_nvp("user_manager", user_manager),*/
            cereal::make_nvp("valid_choice_map", valid_choice_map)); // serialize records 
        }
    }
    catch(...){
...
int UIManager::show_menu(){
 ...
 bool UIManager::is_valid_choice(int choice){
	...
	auto it = valid_choice_map.find(the_ustatus);  
